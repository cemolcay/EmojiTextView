...
public typealias TextToEmojiMapping = [String : [NSAttributedString]]

public func defaultTextToEmojiMapping() -> TextToEmojiMapping {

  var mapping: TextToEmojiMapping = [:]

  func addKey(_ key: String, value: NSAttributedString, atBeginning: Bool) {
  ...
  for (key, value) in jsonDictionary {
    if let key = key as? String,
      ...
      let value = NSAttributedString(string: emojiCharacter)
      ...
      addKey(key, value: value, atBeginning: true)

      if let keywords = dictionary["keywords"] as? [String] {
        for keyword in keywords {
          addKey(keyword, value: value, atBeginning: false)
        }
      }
    }
  }
